...
def oauth():
    ...
    if not client_id:
        raise StandardError("missing client_id")
    ...
    if not client_app:
        raise StandardError("invalid client_id or client_secret")
    ...
    if grant_type == 'credentials':
        ...
        if not email:
            raise StandardError("missing email")

        password = form.get('password')
        if not password:
            raise StandardError("missing password")
...
#            raise StandardError("missing oauth provider")
#
#        oauth_token = form.get('oauth_token')
#        if not oauth_token:
#            raise StandardError("missing oath token")
#
#        if oauth_provide == 'facebook':
#            auth_data = SocialFacebook.get_auth_data(oauth_token)
#        else:
#            raise StandardError("unsupported oauth provider")
#
...
#        })
        ...
        raise StandardError("unsupported grant type")
